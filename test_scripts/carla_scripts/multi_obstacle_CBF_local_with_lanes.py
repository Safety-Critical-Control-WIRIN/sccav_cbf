...
try:
    ...
    from cbf.obstacles import Ellipse2D, CollisionCone2D, BoundingBox, PolyLane
...
def main():
    ...
    try:
        ...
        vehicle = world.spawn_actor(
            ...
            start_pose)
        vehicle.set_target_velocity(carla.Vector3D(0, 10, 0))
        ego = vehicle
        ...
        front_axle_center = euclid.Vector2((fl_wheel.position.x + fr_wheel.position.x) / 2,
                                           (fl_wheel.position.y + fr_wheel.position.y) / 2) / 100
        ...
        if CodeOptions.PRINT_PHYSICS_PROPERTIES:
            ...
            print(
                f"ego bbox extents: x:{ego.bounding_box.extent.x}, y:{ego.bounding_box.extent.y}, z: {ego.bounding_box.extent.z}")
        #####################################
        ...
        """Specifying the lane boundaries"""
        lane_obstacles = {}
        lane_obstacles_coeff = {}
        lane1 = []
        resolution = 100
        ...
        lane_obstacle_1 = PolyLane.fit_polynomial_curve(x_pts=lane1[:, 0],
                                                        y_pts=lane1[:, 1],
                                                        n=1)
        polylane1 = [lane1[:,0], lane_obstacle_1.evaluate_polynomial(lane1[:,0])]

        lane_obstacles['lane1'] = lane_obstacle_1
        lane_obstacles_coeff['lane1'] = lane_obstacle_1.coeffs
        ...
        with CarlaSyncMode(world, camera_rgb, fps=30) as sync_mode:
            while True:
                ...
                if CBF_MODE == CodeOptions.VELOCITY_CBF:
                    ...
                    s = matrix([ego.get_transform().location.x, ego.get_transform().location.y,
                                ego.get_transform().rotation.yaw, ego.get_velocity().length()])
                    cbf_controller.update_state(s=s)
                ...
                if CBF_MODE == CodeOptions.ACCELERATION_CBF:
                    ...
                    bbox_front_center = euclid.Vector2(
                        obstacle_map.ego_x + obstacle_map.ego_width * np.cos(obstacle_map.ego_yaw) / 2, \
                ...
                if CBF_MODE == CodeOptions.COLLISION_CONE_CBF:
                    ...
                    s = matrix([ego.get_transform().location.x, ego.get_transform().location.y,
                                ego.get_transform().rotation.yaw * DEGREE_TO_RADIANS, ego.get_velocity().length()])
                    cbf_controller.update_state(s=s)
                    u_ref = np.array([0.0, 0.0])
                    ...
                    for actor_id in obstacles_list:
                        ...
                        a_cone = np.hypot(obstacles_list[actor_id].bounding_box.extent.x,
                                          obstacles_list[actor_id].bounding_box.extent.y)
                        obstacle_ccone2d = CollisionCone2D(a_cone, s, s_obs)
                        cbf_controller.obstacle_list2d[actor_id] = obstacle_ccone2d

                    # remove = [k for k in cbf_controller.obstacle_list2d if k not in (obstacles_list.keys() and lane_obstacles.keys())]
                    # for k in remove: del cbf_controller.obstacle_list2d[k]
                    print("Number of obstacles detected = " + str(len(cbf_controller.obstacle_list2d)))
