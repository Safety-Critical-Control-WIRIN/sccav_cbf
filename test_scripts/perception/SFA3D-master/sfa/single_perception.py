...
def parse_test_configs():
    ...
    parser.add_argument('--pretrained_path', type=str,
                        default='C:/Shyam/SFA3D-master/SFA3D-master/checkpoints/fpn_resnet_18/fpn_resnet_18_epoch_300.pth', metavar='PATH',
...
class Detection:
    ...
    def get_detections(self, lidar):
        with torch.no_grad():
            # metadatas, bev_maps, img_rgbs = batch_data
            bev_map = lidar
            input_bev_map = bev_map.to(self.configs.device, non_blocking=True).float()
            t1 = time_synchronized()
            outputs = self.model(input_bev_map)
            ...
            print('\tDone inferencing, time: {:.1f}ms, speed {:.2f}FPS'.format((t2 - t1) * 1000, 1 / (t2 - t1)))

            detections = detections[0]  # only first batch

            return detections

    def generate_obstacle_map(self, lidar):
            detections = self.get_detections(lidar)
            print("Number of obstacles detected = " + str(len(detections[0]) + len(detections[1]) + len(detections[2])))
            bev_map = lidar
            # img_rgb = img
            # Draw prediction in the image
            bev_map = (bev_map.squeeze().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
            bev_map = cv2.resize(bev_map, (cnf.BEV_WIDTH, cnf.BEV_HEIGHT))
            bev_map = draw_predictions(bev_map, detections.copy(), self.configs.num_classes)

            # Rotate the bev_map
            bev_map = cv2.rotate(bev_map, cv2.ROTATE_180)

            """Bounding box depiction on camera images to be added"""
