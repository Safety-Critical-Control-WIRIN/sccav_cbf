...
for i=1:length(t)-1
    ...
    u = u_ref + delta;          % finding the actual control input 
    U(:,i) = u;                 % updating the 
    X(:,i+1) = X(:,i) + (u)*dt; % calculating the next position of the vehicle using the differnece equation of the model
end

% plotting the path traversed by the vehicle 
theta = 0:0.01:2*pi;
figure 
plot(X(1,:),X(2,:))
hold on 
...
% plotting the inputs given to the vehicle
figure 
subplot(2,1,1)
...
% plotting the value of h(x) to verify that the vehicle does not enter the
% obstacle boundary
figure 
